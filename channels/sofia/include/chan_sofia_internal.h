 ...
 *
 * Returns NULL when there is no bridged sofia leg, the far leg has no DataChannel, on contention (trylock
...
/*!
 * \brief Provision the local DTLS-SRTP + ICE-lite state for an OUTBOUND WebRTC
 *        OFFER on pvt->rtp, BEFORE sofia_generate_sdp builds the first SDP.
 *
 * Clones the WebRTC answerer provisioning (sofia_sdp.c WebRTC commit) but with NO
 ...
 * authenticated by our local ufrag/pwd set at ast_rtp_new). The WebRTC video offerer also provisions pvt->vrtp
...
struct sofia_pvt {
	...
	/* This leg is the WebRTC OFFERER (gabpbx dialed a webrtc=yes peer and
	 ...
	 * Distinguishes the offerer from the WebRTC answerer at the answer-parse commit:
	 ...
	 * built for the offer and must not be rebuilt). 0 for the WebRTC answerer path and
	...
	/* multi-m-line WebRTC / video over WebRTC (RFC 3264 §6 + RFC 8843): the WebRTC answer MUST carry one m=
	...
	/* non-BUNDLE video transport: the m=video runs on pvt->vrtp with its OWN
	...
	int webrtc_accepted_video_idx;           /* reject_m index of the accepted video — the accepted-video emit path emits it real, the emit skips ONLY this entry; every OTHER m=video is port-0 reflected (exactly one accepted video, RFC 3264 §6 count). -1 = none */
	 ...
	 *   dc_answer_applied: the far leg's ANSWER accepted our offered DataChannel (its m=application has a nonzero
...
struct sofia_peer {
	AST_DECLARE_STRING_FIELDS(
		...
		AST_STRING_FIELD(disallowed_methods);	/* comma-separated SIP methods blocked from outbound Allow header (chan_sip parity); inherits sofia_cfg.disallowed_methods when empty. Parse-compatibility only: dynamic NUTAG_ALLOW generation deferred. */
		...
		AST_STRING_FIELD(mohsuggest);		/* MOH class suggested to the bridged channel when this peer puts us on hold (inbound dir implemented; outbound Alert-Info deferred) */
	...
	int subscribemwi;
	/* Treat every inbound SDP as new, ignoring the o= session-version (chan_sip parity). Implemented:
	 ...
	 * (chan_sip parity). Parse-compatibility only: chan_sofia has no nua_r_redirect handler
	 * yet, so 3xx is not followed regardless of this flag (LIMITATION in sample.conf).
	 * Default 0; inherits default_promiscredir. */
	int promiscredir;
	/* flag for SDP codec-ptime auto-detection (chan_sip parity). Parse-compatibility only:
	 * the sofia_parse_sdp autoframing gate is not implemented yet, so this has no behavioral
	 ...
	 * chan_sip two-bit encoding). Implemented: CNG enables DSP fax-tone detect; T38
	 ...
	 * retry, then 408). Implemented globally via NTATAG_SIP_T1X64 at nua_create; per-peer
	 ...
	 * retransmit timers A/E (UDP) and G (chan_sip parity). Implemented globally via
...
struct sofia_config {
	...
	int directrtpsetup;        /* parse-compatibility only (early-RTP-bridge not implemented); chan_sip itself defaults DISABLED */
	...
	int compactheaders;        /* parse-compatibility only: sofia-sip has no native compact-emit gate. Default 0 (chan_sip). */
	char disallowed_methods[128]; /* parse-compatibility only string-storage; default empty (sofia-sip NUTAG_APPL_METHOD gates unknown methods natively) */
	...
	int ignore_regexpire;      /* 1 = keep expired registrations in peer->contacts (preserve last-known contact across short trunk outages). Implemented at sofia_expire_contacts_cb. Default 0. */
	...
	int legacy_useroption_parsing; /* parse-compatibility only (sofia-sip has no post-parse URI semicolon-strip hook); default 0. chan_sofia URI parsing already RFC 3261-compliant. */
	...
	int default_ignoresdpversion; /* [general] default for peer ignoresdpversion (implemented: o= version stickiness, RFC 3264 §8); default 0 (=honor version) */
	int default_promiscredir;  /* parse-compatibility only (no nua_r_redirect 3xx-honor handler); default 0 */
	int default_autoframing;   /* parse-compatibility only (ptime auto-detect not implemented); default 0 */
	...
	/* rtautoclear: parse-compatibility only two-piece (no peer-level auto-clear infra; peers persist until reload). Defaults: rtautoclear=120s, _enabled=0. */
	int rtautoclear;
	...
	int matchexternaddrlocally; /* parse-compatibility only (chan_sofia externaddr logic compares peer not our-IP; not implemented); dual-key matchexternaddrlocally/matchexterniplocally; default 0 */
	 ...
	 * cos_sip + tos/cos text = parse-compatibility only (no TPTAG_COS / no text-RTP infra). */
	 ...
	 * [general] useragent= overrides. Implemented via SIPTAG_USER_AGENT_STR at nua_create (sofia emits on every request). */
